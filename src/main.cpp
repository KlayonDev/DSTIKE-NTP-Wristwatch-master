...
*/
String WiFI_Locations[][3] = {{"Casa", "ASUS", "Claudio190300"},
...
// Italian
const char dayNames[7][10]={"Dom","Lun","Mar","Mer","Gio","Ven","Sab"};
const char monthNames[12][6]={"Gen","Feb","Mar","Apr","Mag","Giu","Lug","Ago","Set","Ott","Nov","Dic"};
...
void setup() {
  ...
  Serial.println("[OK] Set Font");
  Watch.drawString(OLED_CENTER_W, OLED_Line_3, "Versione 3");
  Serial.println("[OK] draw Text");
  Watch.updateDisplay();
  ...
  Watch.println("Connessione al WiFi");
...
void loop() {
  ...
  if(!Screen_permanent_on){
    if(Watch.screenState && abs(long(displayOffTimer - millis())) > displayTimeout){
      Watch.screenOff();
    }
  }
  ...
  if(Watch.NavBtn_DOWN.wasPressed()){
    if(!Watch.screenState){
      ...
      if(!WiFi_connection()){
        ...
        Watch.drawString(0, OLED_Line_1,  "Aggiornamento Ora");
        Watch.updateDisplay();
        // get the actual time from NTP Server
        int nTrials = 0;
        do {
        ...
        if(dateTime.tm_year > (2016 - 1900)){
          ...
          Watch.drawString(64, OLED_Line_5,"Ora Aggiornata");
          Watch.updateDisplay();
          Watch.setTextAlignment(TEXT_ALIGN_LEFT);
        } else {
          Watch.println("");
          //Watch.println("- invalid data");
          Watch.println("- data non valida");
        }
      }
      // to trigger the full screen update
      last_minute = 100;
...
bool connect_Wifi(const char * _name, const char * _ssid, const char * _password){
  ...
  Watch.setFont(ArialMT_Plain_10);
  Watch.println(strcat("Connessione a ",_name));
  //Watch.println(_name);
  Serial.println(strcat("Connessione a ",_name));
    ...
    switch(wifi_Status){
      ...
      case WL_NO_SSID_AVAIL:
         // Watch.println("-SSID unavailable");
          Watch.println("-SSID non disponibile");
          break;
      case WL_CONNECT_FAILED:
          //Watch.println("-Connection failed");
          Watch.println("-Connessione fallita");
          break;
      case WL_CONNECTION_LOST:
          //Watch.println("-Connection lost");
          Watch.println("-Connessione persa");
          break;
      case WL_DISCONNECTED:
          //Watch.println("Connection attempt");
          Watch.println("Tentativo di connessione");
          break;
      case WL_IDLE_STATUS:
      ...
      default:
          ...
          Watch.println("-Stato sconosciuto");
          break;
    }
  ...
  if(wifi_Status == WL_CONNECTED){
    ...
    Watch.println("-impossibile connettersi");
    return false;
  }
}
